...
MODEL_PATH = MODELS_DIR / "best_next_day_risk_model_strict.joblib"
...
def get_output_dir() -> Path:
    output_dir = Path(__file__).resolve().parents[1] / "outputs" / "model_analysis_strict"
...
def load_model():
    if not MODEL_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run strict training first with: python -m src.train_model_strict"
        )
...
def save_confusion_matrix(y_true: pd.Series, y_pred: pd.Series, output_dir: Path) -> None:
    ...
    cm_csv_path = output_dir / "confusion_matrix_strict.csv"
    cm_df.to_csv(cm_csv_path)

    fig, ax = plt.subplots(figsize=(8, 6))
    im = ax.imshow(cm, interpolation="nearest")
    ax.set_title("Strict Model Confusion Matrix")
    ...
    cm_png_path = output_dir / "confusion_matrix_strict.png"
    fig.savefig(cm_png_path, dpi=200, bbox_inches="tight")
    plt.close(fig)

    print("[OK] Saved strict confusion matrix CSV to:")
    print(cm_csv_path)
    print("[OK] Saved strict confusion matrix PNG to:")
    print(cm_png_path)
...
def save_classification_report(y_true: pd.Series, y_pred: pd.Series, output_dir: Path) -> None:
    ...
    report_path = output_dir / "classification_report_strict.csv"
    report_df.to_csv(report_path)

    print("[OK] Saved strict classification report to:")
    print(report_path)
...
def save_predictions(
    ...
    predictions_path = output_dir / "test_predictions_detailed_strict.csv"
    results_df.to_csv(predictions_path, index=False)

    print("[OK] Saved strict detailed test predictions to:")
...
def save_feature_importance(importance_df: pd.DataFrame, output_dir: Path, top_n: int = 20) -> None:
    importance_path = output_dir / "feature_importance_strict.csv"
    ...
    ax.set_title(f"Top {top_n} Strict Model Feature Importances")
    ax.set_xlabel("Importance")
    ax.set_ylabel("Feature")
    fig.tight_layout()

    plot_path = output_dir / "feature_importance_top20_strict.png"
    fig.savefig(plot_path, dpi=200, bbox_inches="tight")
    plt.close(fig)

    print("[OK] Saved strict full feature importance table to:")
    print(importance_path)
    print("[OK] Saved strict feature importance chart to:")
    print(plot_path)
...
def save_summary_json(y_true: pd.Series, y_pred: pd.Series, output_dir: Path) -> None:
    ...
    summary_path = output_dir / "analysis_summary_strict.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)

    print("[OK] Saved strict analysis summary to:")
    print(summary_path)
    print("\n[INFO] Strict analysis summary:")
    print(summary)
...
def main() -> None:
    ...
    print("\n[OK] Strict model analysis finished successfully.")
